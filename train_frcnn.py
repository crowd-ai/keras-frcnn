...
parser.add_option(
    ...
    default="pascal_voc")
parser.add_option(
    "-n", "--num_rois", dest="num_rois", help="Number of RoIs to process at once.", default=32)
parser.add_option(
    "--network",
    dest="network",
...
model_rpn.compile(
    optimizer=optimizer, loss=[losses.rpn_loss_cls(num_anchors),
  ...
  while True:
    try:
      ...
      progbar.update(iter_num, [('rpn_cls', np.mean(losses[:iter_num, 0])),
                                ...
                                ('detector_cls',
                                 np.mean(losses[:iter_num, 2])), ('detector_regr',
                                                                  np.mean(losses[:iter_num, 3]))])
